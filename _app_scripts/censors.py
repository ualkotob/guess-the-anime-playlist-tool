...
def set_context(
    ...
    _toggle_censor_bar = toggle_censor_bar
    _toggle_censor_nsfw_bar = toggle_censor_nsfw_bar
...
def apply_censors(time, length):
    ...
    if (censors_enabled or censors_nsfw_enabled) and not _get_mismatch_visuals() and not _get_currently_streaming():
        ...
        if mute_censor_used:
            _target_mute = _get_light_muted() if _get_light_round_started() else _get_disable_video_audio()
...
def open_censor_editor(refresh=False, refresh_only=False):
    ...
    def create_bottom_buttons():
        ...
        def _toggle_censor_from_editor():
            _toggle_censor_bar()
            _toggle_censor_nsfw_bar(censors_enabled)
