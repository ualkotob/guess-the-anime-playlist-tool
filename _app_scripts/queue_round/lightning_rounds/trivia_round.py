...
def generate_anime_trivia(data, display=False):
    if _main.fixed_current_round:
        trivia = _main.fixed_current_round.get("trivia_question", "No trivia found.")
        trivia_answer = _main.fixed_current_round.get("trivia_answer", "None")
    ...
    mal_id = data.get("mal")
    stored_trivia = _main.ai_metadata.get(mal_id, {}).get("trivia") if mal_id else []
    ...
    try:
        ...
        if content and "Question:" in content and "Answer:" in content:
            ...
            if mal_id and answer and all(answer != a for _, a in stored_trivia):
                _main.ai_metadata.setdefault(mal_id, {}).setdefault("trivia", []).append([question, answer])
                _main.save_metadata()
            return question, answer
        else:
            return no_trivia_available()
