...
The manager is the package coordinator: it imports sibling overlay modules
directly so its calls are explicit (no late-bound aliasing), but reads /
writes mutable runtime state via ``main_globals`` so cross-cutting state
(``character_round_answer``, ``fixed_current_round``,
``lightning_mode_settings``, ``current_light_variant``,
``title_light_letters`` / ``title_light_string``) stays in main per
[[state-stays-with-its-readers]].

This module is in flux — Step B1 ports the four low-risk helpers
(``apply_reveal_mode``, ``start_title_round``, ``set_title_light_text``,
``get_char_types_by_popularity``); later steps will add ``toggle_light_mode``,
``unselect_light_modes``, ``get_light_round_time``, and
...
from _app_scripts.playback import music, streaming
...
# ---------------------------------------------------------------------------
main_globals: dict = {}
# Live reference to the main module. Used by the larger functions ported here
# (light_round_transition, update_light_round) to read/write main's globals
# directly as attributes — `_main.light_mode` instead of
# `main_globals.get('light_mode')` — which keeps the bodies readable when the
# original is built around dozens of bare names. Set once by `set_context`.
_main = None
_toggle_coming_up_popup    = None
_configure_style           = None
_unhighlight_selection     = None
_queue_next_lightning_mode = None
_get_file_censors          = None
_selected_mode             = None   # Tk StringVar holding the mode chip label
# clean_up_light_round dependencies (main-only helpers)
_set_progress_overlay         = None
_get_light_progress_bar       = None
_spawn_pulsating_music_note   = None
_toggle_outer_edge_overlay    = None
_stop_stream                  = None
_toggle_mute                  = None
_set_video_frame              = None
_send_scoreboard_command      = None
_bottom_info                  = None
_top_info                     = None


def set_context(*, main_globals, main_module=None,
                toggle_coming_up_popup=None, configure_style=None,
                unhighlight_selection=None, queue_next_lightning_mode=None,
                get_file_censors=None, selected_mode=None,
                set_progress_overlay=None, get_light_progress_bar=None,
                spawn_pulsating_music_note=None,
                toggle_outer_edge_overlay=None, stop_stream=None, toggle_mute=None, set_video_frame=None,
                send_scoreboard_command=None, bottom_info=None, top_info=None):
    g = globals()
    g['main_globals'] = main_globals
    if main_module is not None:
        g['_main'] = main_module
        variety_round._main = main_module
        frame_round._main = main_module
        mismatch_round._main = main_module
        trivia_round._main = main_module
    for key, val in [
        ('_toggle_coming_up_popup', toggle_coming_up_popup),
        ('_configure_style', configure_style),
        ('_unhighlight_selection', unhighlight_selection),
        ('_queue_next_lightning_mode', queue_next_lightning_mode),
        ('_get_file_censors', get_file_censors),
        ('_selected_mode', selected_mode),
        ('_set_progress_overlay', set_progress_overlay),
        ('_get_light_progress_bar', get_light_progress_bar),
        ('_spawn_pulsating_music_note', spawn_pulsating_music_note),
        ('_toggle_outer_edge_overlay', toggle_outer_edge_overlay),
        ('_stop_stream', stop_stream),
        ('_toggle_mute', toggle_mute),
        ('_set_video_frame', set_video_frame),
        ('_send_scoreboard_command', send_scoreboard_command),
        ('_bottom_info', bottom_info),
        ('_top_info', top_info),
    ]:
        if val is not None:
            g[key] = val
...
def apply_reveal_mode(reveal_mode, image_source=None, slide_direction=None,
    ...
    if reveal_mode == 'standard':
        char_answer = copy.copy(main_globals.get('character_round_answer'))
...
def start_title_round():
    fixed_current_round = main_globals.get('fixed_current_round')
    ...
    main_globals['current_light_variant'] = title_mode
...
def set_title_light_text():
    ...
    main_globals['title_light_string'] = title
...
def toggle_light_mode(type=None, queue=True, show_popup=True):
    ...
    popout_buttons_by_name = main_globals.get('popout_buttons_by_name', {}) or {}
    lightning_mode_settings = main_globals.get('lightning_mode_settings', {}) or {}
    light_round_number = main_globals.get('light_round_number', 0) or 0
    ...
    if type is None or light_mode == type or (variety_round.variety_light_mode_enabled and type == 'variety'):
        ...
        _toggle_coming_up_popup(False, "Lightning Round")
    else:
        unselect_light_modes()
        mode = light_modes[type]
        main_globals['light_mode'] = type
        light_mode = type
        if type == 'variety':
            variety_round.variety_light_mode_enabled = True
        else:
            _selected_mode.set(f"{mode.get('icon')} {type.upper()}")
            _configure_style()
            _unhighlight_selection(None, setting=True)
        if queue:
            _queue_next_lightning_mode()
        ...
        if show_popup and light_round_number == 0:
            ...
            _toggle_coming_up_popup(True, f"{light_mode.replace('c.', 'Character')} Lightning Round", mode_desc, pil_banner, queue=True)


def unselect_light_modes():
    main_globals['light_mode'] = None
...
def get_light_round_time():
    fixed_lightning_round_playlist_data = main_globals.get('fixed_lightning_round_playlist_data')
    fixed_current_round                 = main_globals.get('fixed_current_round')
    light_round_length                  = main_globals.get('light_round_length', lightning_settings.LIGHT_ROUND_LENGTH_DEFAULT)
    light_round_answer_length           = main_globals.get('light_round_answer_length', lightning_settings.LIGHT_ROUND_ANSWER_LENGTH_DEFAULT)
    light_mode                          = main_globals.get('light_mode')
    currently_playing                   = main_globals.get('currently_playing', {}) or {}
    ...
    start_time = None
    try_count = 0
    file_censors = _get_file_censors(currently_playing.get('filename')) if (need_censors or need_mute_censors) else None
...
def clean_up_light_round(new_round=False):
    ...
    if new_round:
        main_globals['_light_answer_wall_start'] = None
        main_globals['_light_answer_last_tick']  = None
    ...
    if new_round or not fixed_current_round or not fixed_current_round.get("clip_for_answer"):
        _stop_stream(restore=not new_round)
        state.controls.light_muted = False
        if '_sync_control_globals' in main_globals:
            main_globals['_sync_control_globals']()
        if not disable_video_audio:
            _toggle_mute(False, True)

    mismatch_round.mismatch_visuals            = None
    main_globals['character_round_answer']      = None
    ...
    main_globals['light_speed_modifier']        = 1
    main_globals['light_blind_one_second_count'] = None
    player.set_rate(1)

    # Per-round-start teardown of the always-destroyable overlays (no answer-phase carry-over).
    for overlay in [
        _set_progress_overlay,
        ...
        image_reveal_overlays.toggle_character_pixel_overlay,
        image_reveal_overlays.toggle_character_reveal_overlay,
        _spawn_pulsating_music_note,
        _toggle_outer_edge_overlay,
    ...
    if new_round:
        ...
        if light_mode in ('clip', 'ost') and not _clip_for_answer:
            if lightning_mode_settings.get("_misc_settings", {}).get("framed_video"):
                _set_video_frame(True, answer_phase=True)
            else:
                _set_video_frame(False)
        else:
            _set_video_frame(True, answer_phase=True)
    ...
    for info in [_bottom_info, _top_info]:
        info()
...
def light_round_transition():
    _main._light_answer_wall_start = None
    _main._light_answer_last_tick  = None
    _main.light_round_started = False
    _main._showed_lightning_answer = False
    if state.controls.autoplay_toggle == 2:
        _main.stop()
        return
    _main.video_stopped = True
    state.widgets.player.pause()
    _main.set_countdown()
    if _main.web_server.is_running():
        _main.web_server.clear_timer()
    _main.toggle_title_popup(False, instant=True)
    from _app_scripts.playback import blind_screen as _blind_screen
    _main.set_black_screen(True)
    if _blind_screen._video_frame_active:
        _main.set_video_frame(False)
    clean_up_light_round(new_round=True)
    state.widgets.root.after(500, _main.play_next)
...
# from main unchanged in structure — all bare references to main-resident
# names go through `_main.X`, and the original `global` declarations have
# been dropped (Python writes through `_main.X = …` reach main's module
# globals directly).
# ---------------------------------------------------------------------------
def update_light_round(time):
    ...
    if streaming.currently_streaming and _main.light_round_start_time is not None:
        if streaming.get_stream_wall_start() is not None:
            elapsed = max(0, _main.player.get_time() / 1000.0 - _main.stream_start_time)
        else:
            elapsed = 0
        time = _main.light_round_start_time + elapsed
    if not _main.light_round_start_time and (_main.light_mode == 'frame' or frame_round.frame_light_round_started):
        if time < 1 and not frame_round.frame_light_round_started:
            _main.player.pause()
            frame_round.setup_frame_light_round()
        return
    if (time > 1 or _main._light_answer_wall_start is not None) and _main.light_round_start_time != None and _main.light_round_started:
        ...
        _in_answer_phase = _main._light_answer_wall_start is not None or time >= _main.light_round_start_time + _main.light_round_length
        _now = _main._wall_time()
        if _in_answer_phase and _main._light_answer_wall_start is None:
            _main._light_answer_wall_start = _now
            _main._light_answer_last_tick  = _now
        ...
        if _main._light_answer_wall_start is not None and _main._light_answer_last_tick is not None:
            if not _main.player.is_playing():
                _main._light_answer_wall_start += _now - _main._light_answer_last_tick
        _main._light_answer_last_tick = _now
        _answer_elapsed = (_now - _main._light_answer_wall_start) if _main._light_answer_wall_start is not None else 0
        if _in_answer_phase and _answer_elapsed >= _main.light_round_answer_length:
            light_round_transition()
        elif _in_answer_phase:
            start_str = "next"
            blind_length = _main.lightning_mode_settings.get("blind", {}).get("length", _main.LIGHT_ROUND_LENGTH_DEFAULT)
            BLIND_ONE_SECOND_TIME = 4
            if _main.light_blind_one_second_count != None and _main.light_blind_one_second_count < blind_length:
                if _main.light_blind_one_second_count % BLIND_ONE_SECOND_TIME == 0:
                    _main.light_blind_one_second_count += 1
                    _main.player.pause()
                    _main.set_progress_overlay(_main.light_round_length*100, _main.light_round_length*100)
                    def update_light_blind_count():
                        if not _main.light_blind_one_second_count:
                            return
                        if (_main.light_blind_one_second_count + 1) % BLIND_ONE_SECOND_TIME == 0:
                            _main.player.set_time(round(float(_main.light_round_start_time) * 1000))
                            _main.player.play()
                        _main.light_blind_one_second_count += 1
                        _main.set_countdown(round(blind_length - _main.light_blind_one_second_count))
                    for s in range(BLIND_ONE_SECOND_TIME-1):
                        _main.root.after(1000 * (s+1), update_light_blind_count)
                    _main.set_countdown(round(blind_length - _main.light_blind_one_second_count))
            else:
                if not _main._showed_lightning_answer:
                    _main._showed_lightning_answer = True
                    char_answer = copy.copy(_main.character_round_answer)
                    cover_answer = cover_image_overlay.light_cover_image
                    image_answer_source = None  # source text shown as answer (no header, width_max=0.55)
                    if _main.fixed_current_round and _main.fixed_current_round.get("type") == "image":
                        answer_image_url = (_main.fixed_current_round.get("answer_image_url") or "").strip()
                        answer_show_both = _main.fixed_current_round.get("answer_show_both", False)
                        if answer_image_url:
                            try:
                                answer_image = _main.load_image_from_url(answer_image_url, size=None)
                                if answer_show_both:
                            ...
                            if cover_image_overlay.last_image_source[0] == _main.currently_playing.get("filename") and cover_image_overlay.last_image_source[1]:
                                if _main.fixed_current_round.get("image_source"):
                                    image_answer_source = _main.fixed_current_round.get("image_source")
                                else:
                    ...
                    if mismatch_round.mismatch_visuals:
                    ...
                    elif (streaming.last_streamed[0] == _main.currently_playing.get("filename") and streaming.last_streamed[1] and streaming.last_streamed[1] != "Trailer"
                            and not (_main.fixed_current_round and _main.fixed_current_round.get("clip_source_as_song"))):
                    ...
                    elif cover_image_overlay.last_image_source[0] == _main.currently_playing.get("filename") and cover_image_overlay.last_image_source[1]:
                        if _main.fixed_current_round and _main.fixed_current_round.get("image_source"):
                            domain = _main.fixed_current_round.get("image_source")
                        else:
                        ...
                        top_info_data = None
                    if _main.light_mode == 'ost' and not (_main.fixed_current_round and _main.fixed_current_round.get("clip_for_answer")):
                        _main._show_ost_cover()  # hide video during transition; removed by stop_stream
                    _main.toggle_title_popup(True)
                    _main.set_black_screen(False)
                    update_light_round_number()
                    clean_up_light_round()
                    max_width_size = 0.55 if (_main.light_round_answer_length < 10) else 0.5

                    if synopsis_overlay.synopsis_start_index is not None and _main.fixed_current_round and _main.fixed_current_round.get("overlay_during_answer"):
                        synopsis_overlay.toggle_synopsis_overlay(text=synopsis_overlay.get_light_synopsis_string())
                    if top_info_data:
                        _main.top_info(top_info_data, 20)
                    if char_answer:
                        _main.top_info(char_answer[0], width_max=max_width_size) 
                        character_parts_overlay.toggle_character_image_overlay(char_answer[1])
                    if cover_answer:
                        character_parts_overlay.toggle_character_image_overlay(cover_answer)
                    if trivia_answer:
                        _uses_mc_highlight = (_main.fixed_current_round or {}).get("overlay_during_answer") and synopsis_overlay._mc_last_choices
                        if _uses_mc_highlight:
                            # Correct answer shown via highlighted MC overlay; show header only
                            _ans_header = (_main.fixed_current_round or {}).get("answer_header", "")
                            if _ans_header:
                                _main.top_info(_ans_header, width_max=max_width_size)
                        else:
                            _ans_header = (_main.fixed_current_round or {}).get("answer_header", "")
                            _ans_display = f"{_ans_header}:\n{trivia_answer}" if _ans_header else trivia_answer
                            _main.top_info(_ans_display, width_max=max_width_size)
                    if image_answer_source:
                        _main.top_info(image_answer_source, width_max=max_width_size)
                    ...
                    if (_main.fixed_current_round and _main.fixed_current_round.get("clip_for_answer")
                            and _main.stream_start_time is not None):
                        if _main.fixed_current_round.get("clip_replay_for_answer"):
                            _replay_target_sec = _main.fixed_current_round.get("clip_start_time") or 0
                            _main.player.set_time(round(_replay_target_sec * 1000))
                            _main.player.play()
                        else:
                            _clip_answer_target_ms = int((_main.stream_start_time + _main.light_round_length) * 1000)
                            if abs(_main.player.get_time() - _clip_answer_target_ms) > 2000:
                                _main.player.set_time(_clip_answer_target_ms)
                    ...
                    if (_main.fixed_current_round and _main.fixed_current_round.get("start_time") is not None
                            and _main.fixed_current_round.get("type") not in _seek_types
                            and not _main.fixed_current_round.get("clip_for_answer")):
                        _main.player.set_time(round(_main.fixed_current_round["start_time"] * 1000))
                        _main.player.play()
                ...
                if not _main.light_mode or (_main.fixed_lightning_round_playlist_data and _main.fixed_lightning_round_playlist_data.get("current_index") == _main.fixed_lightning_round_playlist_data.get("round_count") - 1):
                    start_str = "end"
                _main.set_countdown(start_str + " in..." + str(round(_main.light_round_answer_length - _answer_elapsed)))
        else:
            time_left = (_main.light_round_length-(time - _main.light_round_start_time))
            if time_left < 1 and _main.light_speed_modifier != 1:
                _main.light_speed_modifier = 1
                _main.player.set_rate(_main.light_speed_modifier)
            if _main.current_light_mode == 'song' or song_overlay.song_overlay_boxes:
                show_title_time = _main.light_round_length if not _main.fixed_current_round else _main.fixed_current_round.get("song_title_reveal_time", _main.light_round_length)
                show_artist_time = _main.light_round_length * 0.75 if not _main.fixed_current_round else _main.fixed_current_round.get("song_artist_reveal_time", _main.light_round_length * 0.75)
                show_slug_time = _main.light_round_length * 0.5 if not _main.fixed_current_round else _main.fixed_current_round.get("song_slug_reveal_time", _main.light_round_length * 0.5)
                show_music_time = _main.light_round_length * (1/3) if not _main.fixed_current_round else _main.fixed_current_round.get("song_music_reveal_time", _main.light_round_length * (1/3))
                ...
                _main.player.audio_set_mute(time_left > show_music_time)
                _main.play_background_music(time_left > show_music_time)
            elif clues_overlay._clues_cell_values:
                data = _main.currently_playing.get("data")
                if time_left <= 15:
                    clues_overlay._clues_cell_values["Tags"] = "\n".join(_main.get_tags(data))
                else:
                    clues_overlay._clues_cell_values["Tags"] = f"in...{round(time_left-15)}"
                if time_left <= 10:
                    clues_overlay._clues_cell_values["Episodes"] = _main.get_episode_display(data, suffix="")
                ...
                if time_left <= 5:
                    clues_overlay._clues_cell_values["Song"] = f"{data.get('slug')}: {_main.get_song_string(data)}"
                else:
                ...
                if trivia_round.light_trivia_answer:
                    answer_time = 6
                synopsis_words = synopsis_overlay.get_light_synopsis_string().split(" " if _main.fixed_current_round else None)
                _reveal_speed = _main.fixed_current_round.get("reveal_speed") if _main.fixed_current_round else None
                if _reveal_speed is not None:
                    # reveal_speed = seconds from start at which full text is shown; 0 = show all immediately
                    elapsed = _main.light_round_length - time_left
                    if _reveal_speed <= 0:
                        progress = 1.0
                    else:
                        progress = max(0.0, min(1.0, elapsed / _reveal_speed))
                else:
                    reveal_duration = _main.light_round_length - answer_time
            ...
            elif _main.current_light_mode == 'emoji' or emoji_overlay.emoji_overlay_window:
                # Reveal emojis one by one over the round
                emojis = emoji_overlay.get_emoji_clues_for_title(_main.currently_playing.get("data"))
                elapsed = _main.light_round_length - max(0, time_left)
                progress = max(0, min(1, elapsed / _main.light_round_length))
                emoji_count = max(1, round(len(emojis) * progress)+1)
                emoji_overlay.toggle_emoji_overlay(emojis=emojis, max_emojis=emoji_count)
            elif _get_light_progress_bar():
                _main.set_progress_overlay(round((time - _main.light_round_start_time)*100), _main.light_round_length*100)
                if streaming.currently_streaming and (not _main.fixed_current_round or _main.fixed_current_round.get("reveal_title_halfway")):
                    half_time = (_main.light_round_length / 2)
                    track_name = _main.extract_track_name_from_youtube_title(streaming.last_streamed[1], _main.currently_playing.get("data", {}))
                    if track_name.strip() != "":
                        if time_left >= half_time:
                            _main.bottom_info(f"TRACK NAME in...{round(time_left-half_time)}")
                        else:
                            _main.bottom_info(track_name)
            elif (_main.current_light_mode == 'title' and _main.current_light_variant == 'reveal') or title_overlay.title_overlay_window:
                if _main.fixed_current_round and _main.fixed_current_round.get("reveal_letter_order"):
                    starting_letters = int(_main.fixed_current_round.get("reveal_starting_count", 0))
                    final_count = len(_main.fixed_current_round.get("reveal_letter_order", "").split(","))
                    reveal_count = final_count - starting_letters
                    interval = reveal_count / 5 if reveal_count > 0 else 1
                else:
                    starting_letters = min(5, max(1, len(_main.title_light_letters) // 5))
                    interval = len(_main.title_light_letters) * 0.09
                    final_count = round((5*interval)+starting_letters)
                word_num = min(final_count, int(((_main.light_round_length-time_left)/3)*interval)+starting_letters)
                _main.bottom_info(f"{word_num}/{final_count} REVEALS", inverse=_main.character_round_answer)
                title_overlay.toggle_title_overlay(title_overlay.get_title_light_string(word_num))
            elif scramble_overlay.scramble_overlay_root or _main.current_light_variant == 'scramble':
                if _main.fixed_current_round and _main.fixed_current_round.get("scramble_place_order"):
                    order_count = len([x for x in _main.fixed_current_round["scramble_place_order"].split(",") if x.strip().isdigit()])
                ...
                placement_cutoff = _main.light_round_length * (2 / 3)

                if time_left >= _main.light_round_length - placement_cutoff:
                ...
                _main.bottom_info(f"{word_num}/{total_letters} PLACEMENTS", inverse=_main.character_round_answer)
                scramble_overlay.toggle_scramble_overlay(num_letters=word_num)
            elif swap_overlay.swap_overlay_root or _main.current_light_variant == 'swap':
                ...
                swap_cutoff = _main.light_round_length * (2 / 3)

                if time_left >= _main.light_round_length - swap_cutoff:
                    # Still in swap phase
                    elapsed = _main.light_round_length - time_left
                ...
                _main.bottom_info(f"{word_num}/{total_swaps} SWAPS", inverse=_main.character_round_answer)
                swap_overlay.toggle_swap_overlay(num_swaps=word_num)
            elif peek_overlay.peek_overlay1:
                gap = _main.get_peek_gap(_main.currently_playing.get("data"))
                _main.toggle_peek_overlay(direction=peek_dispatch.peek_light_direction, progress=((_main.light_round_length-time_left)/_main.light_round_length)*100, gap=gap)
                music.now_playing_background_music(music.music_files[music.current_music_index])
            elif edge_overlay.edge_overlay_box:
                edge_max = max(15, min(70, (_main.currently_playing.get("data").get('popularity') or 3000)/12))
                progress = (_main.light_round_length - time_left) / _main.light_round_length
                block_percent = 100 - (edge_max * progress)  # from 100% to 80%
                _main.toggle_edge_overlay(block_percent=block_percent)
                music.now_playing_background_music(music.music_files[music.current_music_index])
            elif grow_overlay.grow_overlay_boxes:
                grow_max = max(20, min(60, (_main.currently_playing.get("data").get('popularity') or 3000)/10))
                progress = (_main.light_round_length - time_left) / _main.light_round_length
                block_percent = 100 - (grow_max * progress)  # from 100% to 80%
                _main.toggle_grow_overlay(block_percent=block_percent, position=grow_overlay.grow_position)
                music.now_playing_background_music(music.music_files[music.current_music_index])
            elif filter_overlay.filter_vf_active:
                progress = (_main.light_round_length - time_left) / _main.light_round_length
                progress = min(max(progress, 0.0), 1.0)
                _main.toggle_filter_vf(filter_overlay._filter_vf_variant, progress)
                ...
                reveal_num = min(4, (int(_main.light_round_length - (time_left)) // (_main.light_round_length // 4)) + 1)
                characters_overlay.toggle_characters_overlay(num_characters=reveal_num)
                _main.bottom_info(f"{reveal_num}/{4}", inverse=_main.character_round_answer)
                ...
                step_num = min(total_steps-1, (int(_main.light_round_length - (time_left)) // (_main.light_round_length // total_steps)))
                image_reveal_overlays.toggle_character_pixel_overlay(step=step_num)
                _main.bottom_info(f"{step_num+1}/{total_steps}", inverse=_main.character_round_answer)
            elif image_reveal_overlays.blur_reveal_image_window:
                progress = (_main.light_round_length - max(0, time_left)) / _main.light_round_length
                progress = min(max(progress, 0.0), 1.0)
                ...
                _main.bottom_info(f"BLUR: {round(blur_percent)}%", inverse=_main.character_round_answer)
            elif image_reveal_overlays.zoom_reveal_image_window:
                progress = ((_main.light_round_length - 1) - max(0, time_left-1)) / (_main.light_round_length-1)
                progress = min(max(progress, 0.0), 1.0)
                ...
                _main.bottom_info(f"ZOOM OUT: {round(100 - zoom_percent)}%", inverse=_main.character_round_answer)
            elif image_reveal_overlays.slice_overlay_window:
                progress = (_main.light_round_length - max(0, time_left-1)) / _main.light_round_length
                progress = min(max(progress, 0.0), 1.0)
                ...
                _main.bottom_info(f"{slices_to_show}/{total_parts}", inverse=_main.character_round_answer)
                ...
                if not image_reveal_overlays.tile_overlay_swap:
                    ...
                    _main.bottom_info(f"{tiles_to_show}/{total_parts}", inverse=_main.character_round_answer)
                else:
                    ...
                    _main.bottom_info(f"{swaps_to_show}/{total_swaps} SWAPS", inverse=_main.character_round_answer)
                ...
                max_bio_time = 12
                if time_left > _main.light_round_length - max_bio_time:
                    partial = 1 - ((time_left - (_main.light_round_length - max_bio_time)) / max_bio_time)
                ...
                if image_countdown > 0:
                    _main.bottom_info(f"IMAGE IN {image_countdown}...", inverse=True)
                else:
                    _main.bottom_info()
                ...
                light_round_length_t = _main.light_round_length - 5
                ...
                _main.bottom_info(f"{tags_num}/{final_count}")
                ...
                reveal_num = min(6, (int(_main.light_round_length - time_left) // (_main.light_round_length // total_eps)) + 1)
                episode_overlay.toggle_episode_overlay(reveal_num)
                _main.bottom_info(f"{reveal_num}/{total_eps}", inverse=_main.character_round_answer)
            if edge_overlay.edge_overlay_box:
                _main.set_countdown(round(time_left/_main.light_speed_modifier), position="center")
            elif _main.light_blind_one_second_count is not None:
                blind_length = _main.lightning_mode_settings.get("blind", {}).get("length", _main.LIGHT_ROUND_LENGTH_DEFAULT)
                _main.set_countdown(round(blind_length - _main.light_blind_one_second_count))
            else:
                _main.set_countdown(round(time_left/_main.light_speed_modifier), inverse=_main.character_round_answer)
    if _main.light_mode and _main._light_answer_wall_start is None:
        if time < 1:
            _main.toggle_coming_up_popup(False, "Lightning Round")
        if not _main.light_round_started and time < 1:
            _main.light_round_started = True
            _main._showed_lightning_answer = False
            _main.current_light_mode = _main.light_mode
            if _main.lightning_mode_settings.get("_misc_settings", {}).get("framed_video"):
                _main.root.after(300, _main.set_video_frame, True)
            if _main.light_mode in ['regular', 'reveal']:
                _main.root.after(500, _main.set_black_screen, False)
            def set_double_speed():
                _main.light_speed_modifier = 2
                _main.player.set_rate(_main.light_speed_modifier)
                _main.light_round_length = _main.light_round_length * _main.light_speed_modifier
                _main.bottom_info(f"x{_main.light_speed_modifier} SPEED")
            def set_one_second():
                _main.light_blind_one_second_count = 0
                _main.light_round_length = 1
                _main.bottom_info("ONE SECOND")
            if _main.light_round_start_time is None:
                _main.light_round_start_time = get_light_round_time()
            if _main.lightning_mode_settings.get(_main.light_mode, {}).get("muted"):
                _main.toggle_mute(True)
            if _main.light_mode == 'blind':
                if _main.fixed_current_round:
                    blind_mode = _main.fixed_current_round.get("blind_variant", "standard")
                    if _main.fixed_current_round.get("blind_header"):
                        _main.top_info(_main.fixed_current_round.get("blind_header"))
                else:
                    blind_variants = _main.lightning_mode_settings.get("blind",{}).get("variants",{})
                ...
                if blind_mode == "mismatch":
                    ...
                    if not _main.light_round_started:
                        _main.light_round_started = True
                        _main.set_progress_overlay(0, _main.light_round_length * 100)
                    ...
                    _main.player.audio_set_mute(True)
                    def _start_mismatch_after_theme(theme_path):
                        if _main.currently_playing.get("filename") != _cur_fn:
                            _main.player.audio_set_mute(False)
                            return
                        if not theme_path:
                            _main.player.audio_set_mute(False)
                            return
                        ...
                        try:
                            mismatch_round._mismatch_orig_vid = _main.player._p.vid
                            _was_paused = _main.player._p.pause
                            if not _was_paused:
                                _main.player._p.pause = True
                            _main.player._p.command('video-add', theme_path, 'select')
                            # Seek to round start while still paused — no audio disruption
                            try:
                                _main.player._p.seek(float(_main.light_round_start_time), 'absolute')
                            except Exception:
                                pass
                            # Find the newly added external video track ID
                            mismatch_round._mismatch_vid_track_id = None
                            try:
                                tracks = _main.player._p.track_list or []
                        ...
                        _main.spawn_pulsating_music_note()
                        _main.set_black_screen(False)
                        _main.top_info("MISMATCHED VISUALS")
                        _main.bottom_info("GUESS BY MUSIC ONLY")
                        update_light_round_number()
                    _cur_fn = _main.currently_playing.get("filename")
                    def _theme_worker():
                        theme_path = _main.directory_files.get(mismatch_round.get_mismatched_theme())
                        _main.root.after(0, _start_mismatch_after_theme, theme_path)
                    threading.Thread(target=_theme_worker, daemon=True).start()
                else:
                    if not _main.fixed_current_round:
                        is_op = mismatch_round.is_slug_op(_main.currently_playing.get('data', {}).get('slug', ""))
                        ...
                        if double_speed or one_second:
                            if not standard:
                                ...
                                popularity = _main.currently_playing.get("data", {}).get("popularity", 1000) or 3000
            ...
            elif _main.light_mode == 'trivia':
                trivia_data = lightning_queue_data.get(_main.currently_playing.get("filename", {}), {}).get("trivia", [])
                trivia_round.set_light_trivia(trivia_data=trivia_data)
                synopsis_overlay.toggle_synopsis_overlay(text=synopsis_overlay.get_light_synopsis_string(words = 1))
            elif _main.light_mode == "emoji":
                emoji_overlay.toggle_emoji_overlay(max_emojis=1)
            elif _main.light_mode == 'title':
                start_title_round()
                top_header = ""
                if _main.fixed_current_round:
                    top_header = _main.fixed_current_round.get("title_header", "")
                else:
                    top_header = "MUST SAY FULL TITLE"
                _main.top_info(top_header.upper())
            elif _main.light_mode == 'reveal':
                _main.send_scoreboard_command("hide")
                peek_mode = _main.get_next_peek_mode()
                if peek_mode == 'edge':
                    _main.toggle_edge_overlay()
                elif peek_mode == 'grow':
                    grow_overlay.set_grow_position()
                    _main.toggle_grow_overlay(position=grow_overlay.grow_position)
                elif peek_mode == 'slice':
                    peek_dispatch.choose_peek_direction()
                    _main.toggle_peek_overlay()
                    ...
                    _main.toggle_filter_vf(peek_mode, 0)
            elif _main.light_mode == 'characters':
                characters_overlay.characters_round_characters = lightning_queue_data.get(_main.currently_playing.get("filename", {}), {}).get("characters", [])
                ...
                _main.top_info("CHARACTERS")
            elif _main.light_mode in ['cover', 'image']:
                header = ""
                reveal_mode = ""
                if _main.light_mode == 'cover':
                    cover_image_overlay.get_light_cover_image()
                    if _main.fixed_current_round:
                        header = _main.fixed_current_round.get("cover_header", "")
                        reveal_mode = _main.fixed_current_round.get("image_variant", "standard")
                    else:
                        header = "COVER ART"
                        reveal_mode = cover_image_overlay.get_next_cover_reveal_mode()
                elif _main.light_mode == 'image':
                    cover_image_overlay.get_light_image_from_google()
                    if _main.fixed_current_round:
                        header = _main.fixed_current_round.get("image_header", "")
                        reveal_mode = _main.fixed_current_round.get("image_variant", "standard")
                    else:
                        header = "RANDOM IMAGE"
                        reveal_mode = cover_image_overlay.get_next_image_reveal_mode()
                fixed_round = _main.fixed_current_round or {}
            ...
            elif 'c.' in _main.light_mode:
                _main.top_info("GUESS THE CHARACTER", inverse=True)
                _main.character_round_answer = lightning_queue_data.get(_main.currently_playing.get("filename", {}), {}).get("character_answer")
                if not _main.character_round_answer:
                    min_desc = 0
                    if _main.light_mode == 'c. profile':
                        min_desc = 120
                    character_parts_overlay.get_character_round_image(types=get_char_types_by_popularity(mode=_main.light_mode), min_desc_length=min_desc)
                if _main.light_mode == 'c. reveal':
                ...
                elif _main.light_mode == 'c. profile':
                    profile_overlay.toggle_character_profile_overlay()
                elif _main.light_mode == 'c. name':
                    _main.top_info("MUST SAY FULL NAME", inverse=True)
                    start_title_round()
                if _main.lightning_mode_settings.get(_main.light_mode).get("muted"):
                    music.now_playing_background_music(track = None)
                    _main.toggle_mute(True)
            elif _main.light_mode == 'tags':
                tag_cloud_overlay.set_cloud_tags()
                _main.top_info("TAGS")
                tag_cloud_overlay.toggle_tag_cloud_overlay(1)
            elif _main.light_mode == 'episodes':
                episode_overlay.set_light_episodes()
                episode_overlay.toggle_episode_overlay(1)
                if _main.fixed_current_round:
                    top_header = _main.fixed_current_round.get("episodes_header", "")
                else:
                    top_header = "EPISODE TITLES"
                _main.top_info(top_header.upper())
            elif _main.light_mode == 'names':
                episode_overlay.light_name_overlay = True
                episode_overlay.set_light_names()
                episode_overlay.toggle_episode_overlay(1)
                _main.top_info("CHARACTER NAMES")
            elif _main.light_mode in ['clip', 'ost']:
                _always_dl_clip = _main.ffmpeg_available and _main.lightning_mode_settings.get("_misc_settings", {}).get("always_download_clip", False)
                def _ensure_clip_downloaded(yt_url):
                    ...
                    if vid_id and vid_id not in youtube_control._yt_cache_downloads_in_progress:
                        cache_mb = int(_main.lightning_mode_settings.get("_misc_settings", {}).get("download_cache_mb", 0))
                ...
                if _main.fixed_current_round:
                    url, name, channel = _main.fixed_current_round.get("clip_url"), _main.fixed_current_round.get("clip_title"), _main.fixed_current_round.get("clip_author")
                    _ensure_clip_downloaded(url)
                    length = _main.stream_url(url, name, channel)
                else:
                    clip_variants = _main.lightning_mode_settings.get("clip", {}).get("variants", {})
                    clip_enabled, trailer_enabled = clip_variants.get("random_clip"), clip_variants.get("trailer")
                    length = 0
                    is_ost = (_main.light_mode == 'ost')
                    if is_ost:    
                        url = lightning_queue_data.get(_main.currently_playing.get("filename", {}), {}).get("ost_url")
                    else:
                        url = lightning_queue_data.get(_main.currently_playing.get("filename", {}), {}).get("clip_url")
                    if not url:
                        if not streaming.youtube_api_limited and _main.YOUTUBE_API_KEY and (clip_enabled or is_ost):
                            length = _main.play_random_clip(ost=is_ost)
                        elif _main.currently_playing.get("data", {}).get("trailer") and trailer_enabled and is_ost:
                            length = _main.play_trailer()
                    elif trailer_enabled and url[1] == 'trailer':
                        length = _main.play_trailer()
                    else:
                        _ensure_clip_downloaded(url[0])
                        length = _main.stream_url(url[0], url[1], url[2])
                if streaming.currently_streaming:
                    # Compute start time now if we have a known length; otherwise defer until player is ready
                    if length > 0:
                        _main.stream_start_time = _main.get_stream_start_time(length)
                    else:
                        _cst = _main.fixed_current_round.get("clip_start_time") if _main.fixed_current_round else None
                        _main.stream_start_time = max(0, _cst) if _cst is not None else 0
                    _main.test_print(f"Length: {length} | Stream Start Time: {_main.stream_start_time}")
                    # player is already loading the stream (set_media called in stream_url)
                    _main.test_print(streaming.currently_streaming)
                    def wait_for_stream(filename, count):
                        _main.test_print(F"Waiting...{count}")
                        def restart_player():
                            _main.player.stop()
                            _main.player.play()
                            _main.root.after(100, wait_for_stream, filename, 0)
                        if filename != _main.currently_playing.get("filename") or not streaming.currently_streaming:
                            return
                        elif _main.player.is_playing() and _main.player.get_length() > 0:
                            # If length was unknown when stream started, calculate start time now
                            actual_length = _main.player.get_length() / 1000
                            _cst = _main.fixed_current_round.get("clip_start_time") if _main.fixed_current_round else None
                            if _main.stream_start_time == 0 and not (_main.fixed_current_round and _cst is not None):
                                _main.stream_start_time = _main.get_stream_start_time(actual_length)
                                _main.test_print(f"Deferred stream start time: {_main.stream_start_time} (length={actual_length})")
                            def stream_overlay():
                                from _app_scripts.playback import blind_screen as _blind_screen
                                if _main._light_answer_wall_start is not None or not _main.light_round_started:
                                    return  # answer phase already started — don't re-apply overlays
                                if _main.light_mode == 'ost':
                                    # Per-round framed_video override (fixed rounds only)
                                    _framed_ost = (_main.fixed_current_round.get("framed_video")
                                                   if _main.fixed_current_round and _main.fixed_current_round.get("framed_video") is not None
                                                   else False)
                                    if _framed_ost and not _blind_screen._video_frame_active:
                                        _main.set_video_frame(True)
                                    if not _blind_screen.black_overlay:  # already up — skip re-application
                                        _main.set_black_screen(True)  # OST is audio-only; hide video with blind overlay
                                    _top_font_size_ost = 80
                                    if _main.fixed_current_round and _main.fixed_current_round.get("ost_header"):
                                        _main.top_info(_main.fixed_current_round.get("ost_header").upper(), _top_font_size_ost)
                                    else:
                                        _main.top_info("SOUNDTRACK / OST", _top_font_size_ost)
                                else:
                                    ...
                                    _framed_clip = (_main.fixed_current_round.get("framed_video")
                                                    if _main.fixed_current_round and _main.fixed_current_round.get("framed_video") is not None
                                                    else _main.lightning_mode_settings.get("_misc_settings", {}).get("framed_video_clip"))
                                    if _framed_clip and not _blind_screen._video_frame_active:
                                        _main.set_video_frame(True)
                                    _main.set_black_screen(False)
                                    _top_font_size = 80 if _framed_clip else 40
                                    if (_main.fixed_current_round and _main.fixed_current_round.get("censor_bottom")) or (not _main.fixed_current_round and streaming.last_streamed[3] and "Crunchyroll" in streaming.last_streamed[3]):
                                        _main.toggle_outer_edge_overlay()
                                    if _main.fixed_current_round and _main.fixed_current_round.get("clip_header"):
                                        _main.top_info(_main.fixed_current_round.get("clip_header").upper(), _top_font_size)
                                    elif streaming.last_streamed[1] == "Trailer":
                                        _main.top_info("TRAILER", _top_font_size)
                                    else:
                                        _main.top_info("RANDOM CLIP", _top_font_size)
                                update_light_round_number()
                            def set_stream_start():
                                ...
                                if _main.light_mode == 'ost' and not _blind_screen.black_overlay:
                                    _main.set_black_screen(True)
                                _main.player.set_time(round(float(_main.stream_start_time) * 1000))
                                ...
                                if _main.light_mode == 'ost':
                                    _main.set_progress_overlay(0, _main.light_round_length*100)
                                volume_adjustment = _main.fixed_current_round.get("volume_adjustment", 0) if _main.fixed_current_round else 0
                                _stream_volume_level = int(state.controls.volume_level * (1 + ((state.controls.stream_volume_boost + volume_adjustment) / 100)))
                                _main.player.audio_set_volume(_stream_volume_level)
                            def start_player():
                                if filename != _main.currently_playing.get("filename") or not streaming.currently_streaming:
                                    return
                                if _main._light_answer_wall_start is not None or not _main.light_round_started:
                                    return  # answer phase already started — don't interfere
                                target_ms = round(float(_main.stream_start_time) * 1000)
                                current_ms = _main.player.get_time()
                                _main.test_print(f"start_player check: current={current_ms} target={target_ms}")
                                # If the player is more than 2.5 s behind the target the seek
                                # didn't take (stream playing from 0).  Re-attempt once.
                                if _main.player.is_playing() and current_ms < target_ms - 2500:
                                    _main.test_print(f"Seek miss — retrying seek to {target_ms}")
                                    _main.player.set_time(target_ms)
                                    def _check_retry():
                                        if filename != _main.currently_playing.get("filename") or not streaming.currently_streaming:
                                            return
                                        if _main.player.is_playing() and _main.player.get_time() < target_ms - 2500:
                                            _main.test_print("Seek retry failed — restarting stream")
                                            restart_player()
                                    _main.root.after(2000, _check_retry)
                                # else: playing from (approximately) the right position
                            _main.root.after(2000, start_player)
                            set_stream_start()
                            for time in [500, 1000, 1500, 2000]:
                                _main.root.after(time, stream_overlay)
                        elif count >= 5000:
                            restart_player()
                        else:
                            count += 100
                            _main.root.after(100, wait_for_stream, filename, count)
                    wait_for_stream(_main.currently_playing.get("filename"), 0)
                else:
                    if variety_round.last_variety_forced:
                        variety_round.variety_mode_cooldown_counts['clip'] = _main.lightning_mode_settings.get("clip", {}).get("variety", {}).get("cooldown", {}).get("max_gap", 0)
                    _main.toggle_mute(False)
                    _main.root.after(500, _main.set_black_screen, False)
            # Auto-trigger MC bonus if this fixed round defines MC choices
            if _main.fixed_current_round and _main.fixed_current_round.get("mc_choice_2"):
                _main.guess_extra("fixed_mc")
            variety_round.append_lightning_history()
            _main.set_countdown(int(_main.light_round_length), inverse=_main.character_round_answer)
            update_light_round_number(inverse=_main.character_round_answer)
        if not streaming.currently_streaming and _main.light_round_started and _main._light_answer_wall_start is None:
            #only if not already at time (skip during answer phase — answer seek may target an earlier position)
            if _main.light_round_start_time is not None and _main.projected_player_time < round(float(_main.light_round_start_time) * 1000):
                _main.player.set_time(round(float(_main.light_round_start_time) * 1000))
...
def queue_next_lightning_mode():
    def queue_next_lightning_mode_worker():
        ...
        if _main.fixed_lightning_queue or _main.fixed_lightning_round_playlist_data:
            ...
            next_index = _main.playlist["current_index"] + 1
            if next_index < len(_main.playlist["playlist"]) and (_main.get_file_path(_main.playlist["playlist"][next_index]) or _main.is_animethemes_stream_file(_main.playlist["playlist"][next_index])):
                next_filename = _main.get_clean_filename(_main.playlist["playlist"][next_index])
            else:
                return
        
        data = _main.get_metadata(next_filename)
        ...
        while not next_mode:
            ...
            if next_mode in ["clip", 'ost']:
                ...
                if next_fixed_round:
                    ...
                    url, length, name, channel = _main.get_youtube_stream_url(clip_url, include_other_info=True)
                    name = next_fixed_round.get("clip_title") or name
                    channel = next_fixed_round.get("clip_author") or channel
                else:
                    if not streaming.youtube_api_limited and _main.YOUTUBE_API_KEY and clip_enabled:
                        url, name, channel = _main.play_random_clip(data, True, ost=(next_mode=='ost'))
                    if url:
                        yt_source_url = url
                        length = _main.get_youtube_stream_url(url)
                ...
                if yt_source_url and _main.ffmpeg_available:
                    cache_mb = int(_main.lightning_mode_settings.get("_misc_settings", {}).get("download_cache_mb", 0))
                    always_dl = _main.lightning_mode_settings.get("_misc_settings", {}).get("always_download_clip", False)
    ...
    if _main.light_mode or _main.fixed_lightning_queue or _main.fixed_lightning_round_playlist_data:
        threading.Thread(target=queue_next_lightning_mode_worker, daemon=True).start()

# ---------------------------------------------------------------------------
# Lightning round number display. Maintains main's `light_round_number`
# state (incremented in main's play_video / reset by stop_all_queues) and
# pushes it to the bottom-right floating text overlay.
# ---------------------------------------------------------------------------
def update_light_round_number(inverse=False):
    if _main.fixed_lightning_round_playlist_data:
        _main.light_round_number = _main.fixed_lightning_round_playlist_data.get("current_index", 0) + 1
        set_light_round_number(f"#{_main.light_round_number}", inverse=inverse)
    else:
        set_light_round_number("#" + str(_main.light_round_number), inverse=inverse)
...
def set_light_round_number(value=None, inverse=False):
    ...
    if value:
        if len(value) >= 5:
            ...
            size = 62
    _main.set_floating_text("Lightning Round Number", value, position="bottom right", size=size, inverse=inverse)
