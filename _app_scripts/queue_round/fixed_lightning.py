...
def is_theme_missing(theme_filename):
    ...
    def refresh_ui():
        ...
        for round_info in state.playback.fl_rounds_list:
            ...
            for rnd in round_info['data'].get('rounds', []):
                if is_theme_missing(rnd.get('theme', '')):
                    has_missing = True
                    break
            if has_missing:
...
def open_round_editor(round_info, manager_window=None, position=None):
    ...
    editor_window.geometry("700x560")
    ...
    def refresh_editor():
        ...
        def move_up(steps=1):
            """Move selected round up by `steps` (clamped to top)"""
            if selected_round_index[0] is None or selected_round_index[0] == 0:
                return
            idx = selected_round_index[0]
            new_idx = max(0, idx - steps)
            rounds.insert(new_idx, rounds.pop(idx))
            selected_round_index[0] = new_idx
            save_rounds()
            refresh_editor()

        def move_down(steps=1):
            """Move selected round down by `steps` (clamped to bottom)"""
            if selected_round_index[0] is None or selected_round_index[0] >= len(rounds) - 1:
                return
            idx = selected_round_index[0]
            new_idx = min(len(rounds) - 1, idx + steps)
            rounds.insert(new_idx, rounds.pop(idx))
            selected_round_index[0] = new_idx
            save_rounds()
            refresh_editor()

        def alphabetize_rounds():
            """Sort rounds alphabetically by display name"""
            if not rounds:
                return
            result = messagebox.askyesno("Confirm Alphabetize",
        ...
        up_row = tk.Frame(right_frame, bg=_BACKGROUND_COLOR)
        up_row.pack(pady=(0, 8))
        up_button = tk.Button(up_row, text="▲ MOVE", font=font_button, bg="black", fg="white",
                             command=move_up, width=8, pady=8)
        up_button.pack(side="left")
        up5_button = tk.Button(up_row, text="▲▲ 5", font=font_button, bg="black", fg="white",
                              command=lambda: move_up(5), width=5, pady=8)
        up5_button.pack(side="left", padx=(4, 0))

        down_row = tk.Frame(right_frame, bg=_BACKGROUND_COLOR)
        down_row.pack(pady=(0, 8))
        down_button = tk.Button(down_row, text="▼ MOVE", font=font_button, bg="black", fg="white",
                               command=move_down, width=8, pady=8)
        down_button.pack(side="left")
        down5_button = tk.Button(down_row, text="▼▼ 5", font=font_button, bg="black", fg="white",
                                command=lambda: move_down(5), width=5, pady=8)
        down5_button.pack(side="left", padx=(4, 0))

        alphabetize_button = tk.Button(right_frame, text="🔤 ALPHABETIZE", font=font_button, bg="black", fg="white",
                                       command=alphabetize_rounds, width=15, pady=8)
        alphabetize_button.pack(pady=(0, 8))
