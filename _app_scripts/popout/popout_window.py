# _app_scripts/popout_window.py
# Popout controls window - extracted from guess_the_anime.py (Step 34).
import os
...
# ---------------------------------------------------------------------------
# Module-level state synced to main file via _w() (popout widgets/fonts/flags)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
_w = None  # lambda(name, value) - syncs assignment to main file globals; returns value
# Injected functions (called by name)
button_seleted = None
get_window_position_and_setup = None
adjust_up_next_height = None
toggle_show_popout_currently_playing = None
toggle_show_popout_up_next = None
toggle_show_popout_metadata = None
_refresh_popout_toggles = None
update_popout_currently_playling = None
up_next_text = None
select_lightning_mode = None
select_difficulty = None
show_youtube_playlist = None
load_youtube_video = None
search_playlist = None
get_title = None
add_search_playlist = None
set_search_queue = None
get_flat_registry = None
# Injected getters (for dynamic main-file reads)
_get_popout_show_metadata = None
_get_popout_layout = None
_get_popout_columns = None
_get_popout_controls_button = None
_get_light_mode_options = None
_get_selected_mode = None
_get_light_mode = None
_get_difficulty_options = None
_get_difficulty_dropdown = None
_get_youtube_queue = None
_get_youtube_playlist = None
# Injected constants
...
def set_context(*, w,
                button_seleted_fn, get_window_position_and_setup_fn,
                adjust_up_next_height_fn,
                toggle_show_popout_currently_playing_fn, toggle_show_popout_up_next_fn,
                toggle_show_popout_metadata_fn,
                refresh_popout_toggles_fn, update_popout_currently_playling_fn,
                up_next_text_fn, select_lightning_mode_fn, select_difficulty_fn,
                show_youtube_playlist_fn, load_youtube_video_fn,
                search_playlist_fn, get_title_fn,
                add_search_playlist_fn, set_search_queue_fn,
                get_flat_registry_fn,
                get_popout_show_metadata, get_popout_layout, get_popout_columns,
                get_popout_controls_button, get_light_mode_options, get_selected_mode,
                get_light_mode, get_difficulty_options, get_difficulty_dropdown,
                get_youtube_queue, get_youtube_playlist,
                popout_search_default, popout_layout_default,
                background_color, highlight_color):
    global _w
    global button_seleted, get_window_position_and_setup, adjust_up_next_height
    global toggle_show_popout_currently_playing, toggle_show_popout_up_next
    global toggle_show_popout_metadata
    global _refresh_popout_toggles, update_popout_currently_playling, up_next_text
    global select_lightning_mode, select_difficulty
    global show_youtube_playlist, load_youtube_video
    global search_playlist, get_title, add_search_playlist, set_search_queue
    global get_flat_registry
    global _get_popout_show_metadata, _get_popout_layout, _get_popout_columns
    global _get_popout_controls_button, _get_light_mode_options, _get_selected_mode
    global _get_light_mode, _get_difficulty_options, _get_difficulty_dropdown
    global _get_youtube_queue, _get_youtube_playlist
    global POPOUT_SEARCH_DEFAULT, POPOUT_LAYOUT_DEFAULT, BACKGROUND_COLOR, HIGHLIGHT_COLOR
    _w = w
    button_seleted = button_seleted_fn
    get_window_position_and_setup = get_window_position_and_setup_fn
    adjust_up_next_height = adjust_up_next_height_fn
    toggle_show_popout_currently_playing = toggle_show_popout_currently_playing_fn
    toggle_show_popout_up_next = toggle_show_popout_up_next_fn
    toggle_show_popout_metadata = toggle_show_popout_metadata_fn
    _refresh_popout_toggles = refresh_popout_toggles_fn
    update_popout_currently_playling = update_popout_currently_playling_fn
    up_next_text = up_next_text_fn
    select_lightning_mode = select_lightning_mode_fn
    select_difficulty = select_difficulty_fn
    show_youtube_playlist = show_youtube_playlist_fn
    load_youtube_video = load_youtube_video_fn
    search_playlist = search_playlist_fn
    get_title = get_title_fn
    add_search_playlist = add_search_playlist_fn
    set_search_queue = set_search_queue_fn
    get_flat_registry = get_flat_registry_fn
    ...
    _get_popout_controls_button = get_popout_controls_button
    _get_light_mode_options = get_light_mode_options
    _get_selected_mode = get_selected_mode
    _get_light_mode = get_light_mode
    _get_difficulty_options = get_difficulty_options
    _get_difficulty_dropdown = get_difficulty_dropdown
    _get_youtube_queue = get_youtube_queue
    _get_youtube_playlist = get_youtube_playlist
...
def create_popout_controls(title="Popout Controls"):
    ...
    # Local aliases for state + read-once items (Step 34)
    ...
    popout_layout = _get_popout_layout()
    popout_columns = _get_popout_columns()
    light_mode_options = _get_light_mode_options()
    selected_mode = _get_selected_mode()
    difficulty_options = _get_difficulty_options()
    ...
    def on_popout_close():
        ...
        button_seleted(_get_popout_controls_button(), False)
        ...
        if itype == "widget":
            ...
            if item_id == "SEARCH ENTRY":
                ...
                def _on_key(event=None, _sv=search_var, _rv=search_results_var,
                    ...
                    search_ops.search_results = search_playlist(q)
                    titles = [get_title(f, f) for f in search_ops.search_results]
                    _rm.clear()
                    ...
                    if sel and sel in _rm:
                        ...
                        if playlist.get("infinite"):
                            if fn in search_ops.search_results:
                                add_search_playlist(search_ops.search_results.index(fn) + 1)
                            ...
                            if fn in search_ops.search_results:
                                set_search_queue(search_ops.search_results.index(fn) + 1)
                    b = popout_buttons_by_name.get("SEARCH QUEUE")
                    if b:
                        button_seleted(b, bool(search_ops.search_queue))
            ...
            # ─ YouTube queue button ──────────────────────────────────────────
                ...
                def _queue_yt(_ym=yt_video_map):
                    ...
                    if sel and sel in _ym:
                        ...
                        keys = list(_get_youtube_playlist().keys())
                        try:
                    ...
                    if b:
                        button_seleted(b, bool(_get_youtube_queue()))
                w = tk.Button(
            ...
            # ─ Metadata toggle button ────────────────────────────────────────
                ...
                w = tk.Button(
                    ...
                    bg=HIGHLIGHT_COLOR if _get_popout_show_metadata() else "black",
                    fg="white",
                )
                widget = w
    ...
    button_seleted(_get_popout_controls_button(), True)
