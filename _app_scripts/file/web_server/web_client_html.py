"""web_client_html.py — embedded web client markup served by web_server.py.

This is the full mobile-friendly HTML/CSS/JS page sent to audience browsers
for live answer submission. It lives in its own module (a plain module-level
string) rather than an external asset file so PyInstaller still bundles it
into the single exe with no --add-data entry or _MEIPASS path handling.

web_server.py imports HTML and passes it to render_template_string.
"""
...
<head>
  ...
  <style>
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    #pq-nav { display: inline-flex; align-items: center; gap: 3px; position: absolute; left: -4px; }
    #pq-nav-label { color: #668; font-size: 0.9em; min-width: 30px; text-align: center; }
    .pq-nav-btn {
      background: #1a1a2a; border: 1px solid #334; color: #668;
      font-size: 0.75em; padding: 1px 5px; border-radius: 3px; cursor: pointer; line-height: 1.4;
    }
    .pq-nav-btn:hover:not(:disabled) { background: #222240; border-color: #446; color: #88a; }
    .pq-nav-btn:disabled { opacity: 0.3; cursor: default; }
    ...
    .hm-item-host { background: #11182e; border-radius: 6px; padding-left: 6px; padding-right: 6px; }
    .hm-name.hm-host { color: #7fe0a0; }
    .hm-ts {
      color: #667;
      font-size: 0.82em;
      margin-left: 6px;
    ...
    #hm-reply-send:hover { background: #2a2a55; color: #def; }
    ...
    #messages-btn:hover { color: #ccc; background: #22223a; }
    #messages-btn.has-unread { border-color: #6699ff; color: #cde; }
    #messages-badge {
      position: absolute; top: -5px; right: -5px;
      min-width: 16px; height: 16px; padding: 0 4px; box-sizing: border-box;
      background: #e0506a; color: #fff; border-radius: 9px;
      font-size: 0.72em; font-weight: bold; line-height: 16px; text-align: center;
      box-shadow: 0 1px 4px rgba(0,0,0,0.5);
    }
    #messages-panel {
      display: none; position: absolute; bottom: calc(100% + 6px); left: 0;
      width: min(92vw, 360px); z-index: 210;
    }
    #player-list-panel {
      display: none; position: absolute; bottom: calc(100% + 6px); left: 0;
    ...
    #kick-btn-msgs { background: #3a1616; color: #ff9a9a; }
    #kick-btn-msgs:hover { background: #572020; color: #ffd0d0; }
    #kick-btn-msg-alerts { background: #142635; color: #8fc8ff; }
    #kick-btn-msg-alerts:hover { background: #1c3a52; color: #cfeaff; }
    ...
    .sc-skip-btn, .sc-buzzer-lock-btn {
    ...
    .sc-buzzer-lock-btn { color: #806060; border-color: #4a2a2a; }
    .sc-buzzer-lock-btn:hover { background: #2a1717; color: #d08a8a; border-color: #9a4a4a; }
    .sc-buzzer-lock-btn.active {
      background: #301010; color: #ffaaaa; border-color: #cc5555;
      box-shadow: 0 0 6px #cc444499;
    }
    #sc-clear-btn {
    ...
    #sc-teams-summary { display: none; }
    .sc-team-header {
      display: flex; align-items: center; gap: 6px;
      margin: 5px 4px 2px; padding: 4px 8px;
      border: 1px solid #343d52; border-radius: 4px;
      background: #20242f; color: #b9c9ff;
      font-family: 'Segoe UI', sans-serif; font-size: 0.72em; font-weight: 700;
    ...
    .ctrl-pinned-space { width: 8px; flex-shrink: 0; }
    ...
    /* Pin order tray (shown inside the popup in edit mode) —
       mirrors the real controls display: same buttons, same size, same layout.
       Buttons are drag-to-reorder (no handle so spacing matches the display);
       breaks and spaces are click-to-remove. */
    #ctrl-pin-order-tray { display:none; flex-direction:row; flex-wrap:wrap; gap:7px; justify-content:center; padding:6px 0 4px; border-bottom:1px solid #1e1e38; margin-bottom:4px; }
    #ctrl-pin-order-tray.active { display:flex; }
    .ctrl-arrange-btn, .ctrl-arrange-btn .ctrl-queue-main, .ctrl-arrange-btn .ctrl-peek-var-trigger { cursor:grab; }
    .ctrl-arrange-btn:active { cursor:grabbing; }
    .ctrl-arrange-dragging { opacity:0.3 !important; }
    .ctrl-arrange-drop-before { box-shadow:-3px 0 0 0 #66f !important; }
    .ctrl-arrange-drop-after  { box-shadow: 3px 0 0 0 #66f !important; }
    /* Break: full-width clickable strip (forces a wrap like the real break) */
    .ctrl-arrange-break { flex-basis:100%; min-width:100%; height:16px; display:flex; align-items:center; justify-content:center; gap:6px; cursor:pointer; color:#556; font-size:0.66em; letter-spacing:0.04em; border-top:1px dashed #2a2a55; border-bottom:1px dashed #2a2a55; background:rgba(40,40,90,0.12); box-sizing:border-box; }
    .ctrl-arrange-break:hover { color:#e99; border-color:#633; background:rgba(120,40,40,0.16); }
    /* Space: clickable strip (same 8px width as the real space) */
    .ctrl-arrange-space { width:8px; flex-shrink:0; align-self:stretch; min-height:24px; cursor:pointer; border:1px dashed #2a2a55; border-radius:3px; background:rgba(40,40,90,0.12); box-sizing:border-box; }
    .ctrl-arrange-space:hover { border-color:#633; background:rgba(120,40,40,0.18); }
...
<body>
  ...
  <div id="kick-overlay" onclick="if(event.target===this)_kickCancel()">
    <div id="kick-box">
      ...
      <div class="kick-opt-btns">
        ...
        <button id="kick-btn-msgs">Block Messages</button>
        <button id="kick-btn-msg-alerts">Block Notifications</button>
  ...
  <div id="question-box" class="box" style="display:none">
    ...
    <div id="prev-question" style="display:none">
      <div id="prev-question-label">PREVIOUS QUESTION<div id="pq-nav" style="display:none"><button class="pq-nav-btn" id="pq-oldest" onclick="_pqHistorySkip('oldest')" title="Oldest">|&#x25C4;</button><button class="pq-nav-btn" id="pq-prev" onclick="_pqHistoryNav(1)" title="Older">&#x25C4;</button><span id="pq-nav-label"></span><button class="pq-nav-btn" id="pq-next" onclick="_pqHistoryNav(-1)" title="Newer">&#x25BA;</button><button class="pq-nav-btn" id="pq-newest" onclick="_pqHistorySkip('newest')" title="Latest">&#x25BA;|</button></div><button id="prev-question-dismiss" onclick="_dismissPrevQuestion()">CLOSE</button></div>
  ...
  <div id="bottom-left-wrap">
    ...
    <div id="messages-wrap" style="display:none">
      <button id="messages-btn" onclick="_toggleMessages()" title="Messages">&#128172;<span id="messages-badge" style="display:none">0</span></button>
      <div id="messages-panel">
        <div id="host-messages-wrap">
          <div id="host-messages-head">
            <span><span id="hm-title-label">Host Inbox</span> (<span id="host-messages-count">0</span>)</span>
          ...
          <div id="hm-reply-row" style="display:none">
            <select id="hm-reply-to" title="Send to"></select>
            <input id="hm-reply-input" maxlength="280" placeholder="Message…" autocomplete="off" />
            <button id="hm-reply-send" onclick="_sendThreadMessage()">Send</button>
          </div>
        </div>
      </div>
    </div>
  ...
  <div id="ctrl-extras-popup-overlay" onclick="if(event.target===this)_ctrlCloseExtrasPopup()">
    <div id="ctrl-extras-popup-box">
      ...
      <div id="ctrl-extras-edit-hint">Click a button below to pin/unpin &middot; Drag pinned buttons above to reorder &middot; Click a space/break to remove it</div>
      <!-- Order tray: a live mirror of the pinned display; buttons drag to reorder -->
      <div id="ctrl-pin-tray-actions">
        <span id="ctrl-pin-tray-label">Pinned layout &mdash; drag buttons to reorder, click a space/break to remove:</span>
      ...
      <div class="ctrl-extras-sect-row">
        ...
        <button class="ctrl-bonus-btn ctrl-toggle-btn ctrl-sect-toggle" data-extra-id="tgl_always_on_top" onclick="_ctrlExtraClick('tgl_always_on_top')" title="Keep the player window on top of all other windows">On Top</button>
      ...
      <div class="ctrl-extras-sect-row">
        ...
        <button class="ctrl-bonus-btn ctrl-sect-reveal" data-extra-id="rev_info" onclick="_ctrlExtraClick('rev_info')">Info Popup</button>
      ...
      <div class="ctrl-extras-sect-row">
        <span class="ctrl-extras-sect-label">Playback &amp; Session</span>
        ...
        <button class="ctrl-bonus-btn ctrl-sect-session" data-extra-id="apply_filter" onclick="_ctrlExtraClick('apply_filter')" title="Apply a saved filter to the current playlist">Filter</button>
      ...
      <div class="ctrl-extras-sect-row">
        ...
        <button class="ctrl-bonus-btn ctrl-sect-scoreboard" data-extra-id="rules_file" onclick="_ctrlExtraClick('rules_file')" title="Select rules file">Rules</button>
    ...
    <div id="controller-box">
      ...
      <div class="ctrl-section" id="ctrl-section-controls">
        ...
        <div id="ctrl-controls-panel" style="display:none">
            ...
            <!-- More… button is rendered as the "_more_" layout token inside here -->
  ...
  <script>
    ...
    let _prevQMyAnswer = '';         // snapshot of player's answer for the currently displayed history entry
    const _prevQHistory = [];        // oldest → newest; each entry: all _prevQ* fields + myAnswer
    let _prevQHistoryIdx = 0;        // 0 = newest; _prevQHistory[len-1-idx] = currently displayed
    ...
    let _buzzerState = { open: false, locked: false, order: [], disabled_names: [] };
    let _hostMessages = [];
    ...
    socket.on('answer_reveal', data => {
      ...
      _prevQMyAnswer     = _myLastAnswer       || '';
      _prevQHistoryPush({
        title: _prevQTitle, correct: _prevQCorrect, type: _prevQType,
        correctTags: _prevQCorrectTags, data: _prevQData,
        allAnswers: _prevQAllAnswers, scoreHints: _prevQScoreHints,
        playerScores: _prevQPlayerScores, myAnswer: _prevQMyAnswer
      });
      _prevQHistoryIdx = 0;
    ...
    function _prevQHistoryPush(entry) {
      _prevQHistory.push(entry);
      if (_prevQHistory.length > 20) _prevQHistory.shift();
    }
    function _prevQLoadEntry(e) {
      _prevQTitle        = e.title;
      _prevQCorrect      = e.correct;
      _prevQType         = e.type;
      _prevQCorrectTags  = e.correctTags;
      _prevQData         = e.data;
      _prevQAllAnswers   = e.allAnswers;
      _prevQScoreHints   = e.scoreHints;
      _prevQPlayerScores = e.playerScores;
      _prevQMyAnswer     = e.myAnswer;
    }
    function _pqUpdateNav() {
      const total = _prevQHistory.length;
      const nav = document.getElementById('pq-nav');
      if (nav) nav.style.display = total > 1 ? 'inline-flex' : 'none';
      const label = document.getElementById('pq-nav-label');
      if (label) label.textContent = total > 0 ? ((total - _prevQHistoryIdx) + ' / ' + total) : '';
      const prev = document.getElementById('pq-prev');
      const next = document.getElementById('pq-next');
      const oldest = document.getElementById('pq-oldest');
      const newest = document.getElementById('pq-newest');
      if (prev)   prev.disabled   = _prevQHistoryIdx >= total - 1 || total <= 1;
      if (next)   next.disabled   = _prevQHistoryIdx <= 0         || total <= 1;
      if (oldest) oldest.disabled = _prevQHistoryIdx >= total - 1 || total <= 1;
      if (newest) newest.disabled = _prevQHistoryIdx <= 0         || total <= 1;
    }
    function _pqHistoryNav(dir) {
      const total = _prevQHistory.length;
      if (total < 2) return;
      _prevQHistoryIdx = Math.max(0, Math.min(total - 1, _prevQHistoryIdx + dir));
      _prevQLoadEntry(_prevQHistory[total - 1 - _prevQHistoryIdx]);
      _pqUpdateNav();
      _showPrevQuestion();
    }
    function _pqHistorySkip(to) {
      const total = _prevQHistory.length;
      if (total < 2) return;
      _prevQHistoryIdx = (to === 'newest') ? 0 : total - 1;
      _prevQLoadEntry(_prevQHistory[total - 1 - _prevQHistoryIdx]);
      _pqUpdateNav();
      _showPrevQuestion();
    }
    ...
    function _kickOptions(player) {
      ...
      const messagesBlocked = !!(player && player.messages_blocked);
      const messageNotificationsBlocked = !!(player && player.message_notifications_blocked);
      const sid = (player && player.sid) ? player.sid : '';
      let _label = 'Action for: ' + name + (player && player.dup_index > 1 ? ' (' + player.dup_index + ')' : '');
      if (sid) {
        const _parts = [];
        if (player.join_ms) {
          const _d = new Date(Number(player.join_ms));
          _parts.push('joined ' + String(_d.getHours()).padStart(2, '0') + ':' + String(_d.getMinutes()).padStart(2, '0'));
        }
        if (player.dup_total > 1) _parts.push(player.same_ip ? 'same IP' : '⚠ different IP');
        if (_parts.length) _label += '  — ' + _parts.join(' · ');
      }
      document.getElementById('kick-player-name').textContent = _label;
      document.getElementById('kick-btn-shadow').onclick = () => { _kickCancel(); socket.emit(sid ? 'remove_device' : 'remove_player', sid ? { sid } : { name }); };
      ...
      document.getElementById('kick-btn-ip').onclick     = () => { _kickCancel(); socket.emit(sid ? 'ip_ban_device' : 'ip_ban', sid ? { sid } : { name }); };
      ...
      const mb = document.getElementById('kick-btn-msgs');
      if (mb) {
        mb.textContent = messagesBlocked ? 'Unblock Messages' : 'Block Messages';
        mb.onclick = () => {
          _kickCancel();
          socket.emit('host_message_block', { name, blocked: !messagesBlocked });
        };
      }
      const nb = document.getElementById('kick-btn-msg-alerts');
      if (nb) {
        nb.textContent = messageNotificationsBlocked ? 'Unblock Notifications' : 'Block Notifications';
        nb.onclick = () => {
          _kickCancel();
          socket.emit('host_message_notifications_block', { name, blocked: !messageNotificationsBlocked });
        };
      }
      ...
      _loadSavedEmojis().forEach(emoji => {
        ...
        if (isSet) {
          ...
          _bindSecondaryAction(btn, () => {
            _confirm('Delete emoji set ' + emoji + '?', 'Delete', () => {
    ...
    function _buzzerLockControlsVisible() {
      let autoBuzzer = false;
      try { autoBuzzer = _ctrlAutoBonusCurrent === 'buzzer'; } catch(e) {}
      return !!(_isBuzzerQuestion || autoBuzzer);
    }
    function _isPlayerBuzzerLocked(name) {
      const locked = Array.isArray(_buzzerState.disabled_names) ? _buzzerState.disabled_names : [];
      return locked.includes(name);
    }
    function _refreshBuzzerLockButtons() {
    ...
    function _pressBuzz() {
      if (!_buzzerState.open || _buzzerState.locked || _isPlayerBuzzerLocked(playerName)) return;
      try {
    ...
    function _updateBuzzerUI() {
      ...
      const isPlayerLocked = _isPlayerBuzzerLocked(playerName);
      const shouldShow = !!_isBuzzerQuestion;

      panel.style.display = shouldShow ? 'block' : 'none';
      if (!shouldShow) return;

      if (isPlayerLocked) status.textContent = 'Your buzzer is locked.';
      else if (isLocked) status.textContent = 'Buzzer locked.';
      ...
      btn.disabled = !inQuestion || !isOpen || isLocked || isPlayerLocked || myIdx >= 0 || !playerName;
    ...
    function _applyBuzzerState(data) {
      _buzzerState = Object.assign({ open: false, locked: false, order: [], disabled_names: [] }, data || {});
      // host button states
    ...
    document.addEventListener('DOMContentLoaded', () => {
      ...
      _ctrlLoadPanels();
      const replyInput = document.getElementById('hm-reply-input');
      if (replyInput) {
        replyInput.addEventListener('keydown', e => {
          if (e.key === 'Enter') _sendThreadMessage();
        });
      }
    ...
    const _PLAYER_COLOR_MATCH_THRESHOLD = 32;

    function _cssColorKey(color) {
      const raw = String(color || '').trim();
      if (!raw) return '';
      const probe = document.createElement('span');
      probe.style.color = raw;
      if (!probe.style.color) return raw.toLowerCase();
    ...
    function _cssColorRgb(color) {
      const key = _cssColorKey(color);
      const m = key.match(/^rgba?\((\d+),\s*(\d+),\s*(\d+)/);
      if (!m) return null;
      return [parseInt(m[1], 10), parseInt(m[2], 10), parseInt(m[3], 10)];
    }

    function _colorsTooClose(a, b) {
      const ar = _cssColorRgb(a);
      const br = _cssColorRgb(b);
      if (!ar || !br) return _cssColorKey(a) === _cssColorKey(b);
      return Math.max(Math.abs(ar[0] - br[0]), Math.abs(ar[1] - br[1]), Math.abs(ar[2] - br[2])) <= _PLAYER_COLOR_MATCH_THRESHOLD;
    }
    ...
    function _nudgeTextColor(bg, text) {
      const bgRgb = _cssColorRgb(bg);
      const textRgb = _cssColorRgb(text);
      if (!bgRgb || !textRgb) return text;
      const targetDelta = _PLAYER_COLOR_MATCH_THRESHOLD + 1;
      const diffs = textRgb.map((v, i) => v - bgRgb[i]);
      const channels = [0, 1, 2].sort((a, b) => Math.abs(diffs[b]) - Math.abs(diffs[a]));

      for (const idx of channels) {
        const direction = diffs[idx] >= 0 ? 1 : -1;
        const target = bgRgb[idx] + direction * targetDelta;
        if (target >= 0 && target <= 255) {
          const nudged = textRgb.slice();
          nudged[idx] = target;
          return _rgbToHex(nudged);
        }
      }
    ...
    function _colorpickSave() {
      ...
      let text = _colorpickTextValue;
      if (bg && text && _colorsTooClose(bg, text)) {
        text = _nudgeTextColor(bg, text);
        _colorpickTextValue = text;
        _showToast('Text color was too close to background, so it was nudged.');
      }
    ...
    function _refreshPlayerList(players) {
      ...
      if (_isHost) _refreshHostReplyOptions();
    }
    ...
    function _updateMsgBadge() {
      ...
      let unread = 0;
      if (!_msgPanelOpen) {
        for (let i = Math.max(0, _msgSeenCount); i < _hostMessages.length; i++) {
          const m = _hostMessages[i];
          const isHostMsg = !!(m && m.sender === 'host');
          // Count only incoming messages (not the ones I sent myself).
          if (_isHost ? !isHostMsg : isHostMsg) unread++;
        }
      }
      if (unread > 0) {
    ...
    function _toggleMessages() {
      const panel = document.getElementById('messages-panel');
      if (!panel) return;
      _msgPanelOpen = !_msgPanelOpen;
      panel.style.display = _msgPanelOpen ? 'block' : 'none';
      if (_msgPanelOpen) {
        _msgSeenCount = _hostMessages.length;
        const list = document.getElementById('host-messages-list');
        if (list) list.scrollTop = list.scrollHeight;
        const inp = document.getElementById('hm-reply-input');
        if (inp) setTimeout(() => { try { inp.focus(); } catch (e) {} }, 0);
      }
      _updateMsgBadge();
    }
    function _renderHostMessages(messages) {
      _hostMessages = Array.isArray(messages) ? messages : [];
      if (_msgPanelOpen) _msgSeenCount = _hostMessages.length;
      const rows = _hostMessages.map(m => {
        const isHostMsg = !!(m && m.sender === 'host');
        const text = _escHtml((m && m.text) ? m.text : '');
        const ts = _fmtHostMsgTime(m && m.ts);
        let label, nameCls;
        if (isHostMsg) {
          nameCls = 'hm-name hm-host';
          const hostName = '👑 ' + ((m && m.name) ? m.name : 'Host');
          if (_isHost) {
            const to = (m && m.to) ? m.to : 'all';
            label = hostName + ' → ' + ((to === 'all' || to === '') ? 'All' : to);
          } else {
            label = hostName;
          }
        } else {
          nameCls = 'hm-name';
          label = _isHost ? ((m && m.name) ? m.name : 'Player') : 'You';
        }
        return '<div class="hm-item' + (isHostMsg ? ' hm-item-host' : '') + '"><span class="' + nameCls + '">'
          + _escHtml(label) + '</span><span class="hm-ts">' + ts + '</span><div>' + text + '</div></div>';
      });
      ...
      // Title + host-only controls
      const titleLabel = document.getElementById('hm-title-label');
      if (titleLabel) titleLabel.textContent = _isHost ? 'Host Inbox' : 'Messages';
      const clearBtn = document.getElementById('host-messages-clear');
      if (clearBtn) clearBtn.style.display = _isHost ? '' : 'none';
      // Unified compose row: dropdown is host-only; players just message the host.
      const replyRow = document.getElementById('hm-reply-row');
      if (replyRow) replyRow.style.display = 'flex';
      const replyTo = document.getElementById('hm-reply-to');
      if (replyTo) replyTo.style.display = _isHost ? '' : 'none';
      const replyInput = document.getElementById('hm-reply-input');
      if (replyInput) replyInput.placeholder = _isHost ? 'Message player(s)…' : 'Message host…';
      if (_isHost) _refreshHostReplyOptions();
      ...
      if (_msgPanelOpen) list.scrollTop = list.scrollHeight;
      _updateMsgBadge();
    }
    function _refreshHostReplyOptions() {
      const sel = document.getElementById('hm-reply-to');
      if (!sel) return;
      const prev = sel.value || 'all';
      const seen = new Set();
      const names = (_lastPlayerList || [])
        .filter(p => p && !p.host && !p.kicked && p.name)
        .map(p => p.name)
      ...
      sel.value = (prev === 'all' || seen.has(prev)) ? prev : 'all';
    }
    function _sendThreadMessage() {
      const inp = document.getElementById('hm-reply-input');
      if (!inp) return;
      ...
      if (_isHost) {
        const sel = document.getElementById('hm-reply-to');
        const to = sel ? (sel.value || 'all') : 'all';
        socket.emit('host_message_reply', { to, text });
      } else {
    ...
    function _showHostMessageToast(entry) {
      const isHostMsg = !!(entry && entry.sender === 'host');
      // Host is notified of player→host messages; players of host→player replies.
      if (_isHost ? isHostMsg : !isHostMsg) return;
      ...
      if (!toast) return;
      const name = _escHtml(isHostMsg ? ('👑 ' + ((entry && entry.name) ? entry.name : 'Host')) : ((entry && entry.name) ? entry.name : 'Player'));
    ...
    function _ctrlLoadPanels() {
      ...
      if (state && Array.isArray(state.pinnedExtras)) {
        ...
        state.pinnedExtras.forEach(raw => {
          ...
          const isLayout = id.startsWith('_br_') || id.startsWith('_sp_') || id === '_more_';
        ...
        // Migration: layouts saved before More… became movable have no token —
        // append it at the end (its old fixed position).
        if (!cleaned.includes('_more_')) cleaned.push('_more_');
    ...
    function _scCleanTeam(team) {
      return String(team || '').trim();
    }
    function _scLooksLikeTeamId(team) {
      return /^\d+$/.test(_scCleanTeam(team));
    }
    function _scTeamStorageKey() {
      return 'scPlayerTeams:' + location.pathname;
    }
    function _scLoadPlayerTeams() {
      try {
        const raw = localStorage.getItem(_scTeamStorageKey());
        const parsed = raw ? JSON.parse(raw) : null;
        _scPlayerTeams = (parsed && typeof parsed === 'object' && !Array.isArray(parsed)) ? parsed : {};
      } catch (e) { _scPlayerTeams = {}; }
    }
    function _scSavePlayerTeams() {
      try { localStorage.setItem(_scTeamStorageKey(), JSON.stringify(_scPlayerTeams || {})); } catch (e) {}
    }
    function _scSetPlayerTeamName(name, team) {
      const clean = _scCleanTeam(team);
      if (!name) return;
      if (clean && !_scLooksLikeTeamId(clean)) _scPlayerTeams[name] = clean;
      else delete _scPlayerTeams[name];
      _scSavePlayerTeams();
    }
    function _scDisplayTeam(playerOrTeam, maybeName) {
      if (playerOrTeam && typeof playerOrTeam === 'object') {
        const name = String(playerOrTeam.name || '').trim();
        const raw = _scCleanTeam(playerOrTeam.team);
        if (raw && !_scLooksLikeTeamId(raw)) {
          _scSetPlayerTeamName(name, raw);
          return raw;
        }
        return '';
      }
      const raw = _scCleanTeam(playerOrTeam);
      if (raw && !_scLooksLikeTeamId(raw)) return raw;
      return '';
    }
    function _scTeamOptions(extraTeam) {
      const byKey = new Map();
      function add(raw) {
        const team = _scCleanTeam(raw);
        if (!team) return;
        const key = team.toLocaleLowerCase();
        if (!byKey.has(key)) byKey.set(key, team);
      }
      (_scTeamNames || []).forEach(add);
      (_scPlayers || []).forEach(p => add(_scDisplayTeam(p)));
      if (_scLastData && Array.isArray(_scLastData.players)) {
        _scLastData.players.forEach(p => add(_scDisplayTeam(p)));
      }
      Object.values(_scPlayerTeams || {}).forEach(add);
      if (Array.isArray(extraTeam)) extraTeam.forEach(add);
      else add(extraTeam);
      return Array.from(byKey.values()).sort((a, b) => a.localeCompare(b, undefined, { sensitivity: 'base' }));
    }
    function _scRememberTeamName(team) {
      const clean = _scCleanTeam(team);
      if (!clean) return;
    ...
    function _scPlayerGroups(players) {
      const groups = [];
      const byKey = new Map();
      const unassigned = [];
      let hasTeams = false;
      (players || []).forEach(p => {
        const team = _scDisplayTeam(p);
        if (!team) { unassigned.push(p); return; }
        hasTeams = true;
        const key = team.toLocaleLowerCase();
        if (!byKey.has(key)) {
      ...
      if (!hasTeams) return [{ team: '', players: players || [] }];
    ...
    function _scMakeTeamHeader(team, players) {
      ...
      const name = document.createElement('span');
      name.className = 'sc-team-header-name';
      name.textContent = team || 'No team';
      const count = document.createElement('span');
      count.className = 'sc-team-header-count';
    ...
    function _scRender(data, forceRebuild = false) {
      ...
      const container    = document.getElementById('sc-players');
      if (!container) return;
      if (_scIsScoreboardEditing()) return;

      const sameDeltaBtns = newDeltaBtns.length === _scDeltaBtns.length &&
        newDeltaBtns.every((d, i) => d.label === _scDeltaBtns[i].label);
      const samePlayers = newPlayers.length === _scPlayers.length &&
        newPlayers.every((p, i) => p.name === _scPlayers[i].name && _scDisplayTeam(p) === _scDisplayTeam(_scPlayers[i]));

      if (!forceRebuild && samePlayers && sameDeltaBtns) {
        _scPlayers = newPlayers;
        _scTeamNames = _scTeamOptions();
      ...
      const groups = _scPlayerGroups(_scPlayers);
      const grouped = groups.length > 1 || (groups[0] && groups[0].team);
      groups.forEach(group => {
        if (grouped) container.appendChild(_scMakeTeamHeader(group.team, group.players));
    ...
    function _scShowRowMenu(name, anchor) {
      ...
      const curTeam = _scDisplayTeam(curPlayer || '', displayName);
      if (curTeam !== '') {
        menuItem('\u274C  Remove team', '#cc9966', () => _scSetTeamInline(displayName, ''));
      }
    ...
    function _scSetTeamInline(name, team) {
      _scSetTeamForPlayers([name], team);
    }

    function _scSetTeamForPlayers(names, team) {
      const clean = _scCleanTeam(team);
      (names || []).forEach(name => {
        socket.emit('host_action', { action: 'player_set_team', name, team: clean });
        const op = _scPlayers.find(p => p.name === name);
        if (op) op.team = clean;
        _scSetPlayerTeamName(name, clean);
      });
      _scRememberTeamName(clean);
      if (_scLastData) _scLastData.players = _scPlayers.map(p => Object.assign({}, p));
      _scOptimisticUntil = Date.now() + 5000;
      _scOptimisticPlayers = _scPlayers.map(p => Object.assign({}, p));
      _scRender(_scLastData || { players: _scPlayers, delta_buttons: _scDeltaBtns.map(d => d.label).join(',') }, true);
    }

    function _scSetTeamPromptForPlayers(labelText, names, currentTeam) {
      ...
      const dlId = 'sc-team-dl';
      const inp = document.createElement('input');
      inp.type = 'text';
      inp.name = 'scoreboard_team_name';
      inp.setAttribute('list', dlId);
      inp.placeholder = 'Team name';
      inp.autocomplete = 'new-password';
      inp.autocapitalize = 'none';
      inp.spellcheck = false;
      inp.value = _scCleanTeam(currentTeam);
      ...
      dl.id = dlId;
      _scTeamOptions(currentTeam).forEach(t => { const opt = document.createElement('option'); opt.value = t; dl.appendChild(opt); });
      ...
      setTimeout(() => { inp.focus(); if (inp.value) inp.select(); }, 0);

      function commit() {
        const team = inp.value.trim();
        wrap.remove();
        _scSetTeamForPlayers(names, team);
      }
    ...
    function _scSetTeamPrompt(name, menuEl) {
      ...
      const currentTeam = _scDisplayTeam(curPlayer || '', name);
      _scSetTeamPromptForPlayers('Team for ' + name, [name], currentTeam);
    }
    ...
    socket.on('scores_update', data => {
      ...
      _scLastData = data;
      // Render whenever the scoreboard div is actually visible, unless an edit field has focus.
      ...
      if (_scViewOpen || (sv && sv.style.display !== 'none' && sv.style.display !== '')) {
        ...
        if (!_scIsScoreboardEditing()) _scRender(data);
      }
      _scApplyViewState();
    });

    socket.on('teams_update', data => {
      if (data && Array.isArray(data.teams)) _scTeamNames = _scTeamOptions(data.teams);
    });
    ...
    const _ctrlLtTypes = [
      ...
      {id:'lightning_characters', mode:'characters', label:'Char'},
    ...
    function _ctrlRenderRulesList(files, selected) {
      ...
      files.forEach(f => {
        ...
        el.className = 'ctrl-popup-item';
        const isNone = !f;
        const isCurrent = isNone ? !selected : (f === selected);
        const label = isNone
          ? '<span class="ctrl-popup-item-title" style="font-style:italic;color:#c96">None</span>'
          : '<span class="ctrl-popup-item-title">' + _ctrlEscapeHtml(f) + '</span>';
        el.innerHTML =
          '<span style="display:inline-block;width:1.2em;color:#4c8;font-size:0.9em">' +
          (isCurrent ? '\u2713' : '') + '</span>' + label;
        if (isCurrent) el.style.cssText = 'background:#1a2a1a;border-left:2px solid #4a8;';
        el.onclick = () => {
    ...
    function _ctrlToggleFilterList() {
      ...
      _ctrlCloseAllLists();
      if (opening) {
        _ctrlFilterListOpen = true;
        _ctrlListPopupOpen('Apply Filter', 'apply_filter');
        const list = document.getElementById('ctrl-list-popup-list');
        if (list) list.innerHTML = '<div style="padding:8px;color:#556;font-size:0.85em">Loading...</div>';
        socket.emit('host_action', {action: 'get_filter_list'});
      }
    }

    socket.on('filter_list', data => {
      if (!_ctrlFilterListOpen) return;
      const list = document.getElementById('ctrl-list-popup-list');
      if (!list) return;
      list.innerHTML = '';
      ...
      if (!filters.length) {
        const empty = document.createElement('div');
        empty.style.cssText = 'padding:8px;color:#556;font-size:0.85em';
        empty.textContent = 'No saved filters found';
        list.appendChild(empty);
        return;
      }
      filters.forEach(name => {
        ...
        el.onclick = () => {
          _ctrlConfirmYesNo(
            ...
            () => {
              socket.emit('host_action', {action: 'select_filter', name: name});
              _ctrlListPopupClose();
            }
          );
        };
    ...
    socket.on('toggles_update', data => {
      ...
      const tglProxyIds = {
        ...
        'rules_file': !!(data.scoreboard_open && data.selected_rules_file),
    ...
    const _ctrlExtrasConfig = {
      ...
      'tgl_always_on_top':{ classes: 'ctrl-toggle-btn ctrl-sect-toggle',       html: 'On Top',    title: 'Keep the player window on top of all other windows' },
      ...
      'apply_filter': { classes: 'ctrl-sect-session',                          html: 'Filter',    title: 'Apply a saved filter to the current playlist' },
    ...
    const _ctrlExtraActions = {
      ...
      'apply_filter':  () => { _ctrlCloseExtrasPopup(); _ctrlToggleFilterList(); },
    ...
    // Append an item, but keep a trailing "More…" token at the very end so
    // it stays last as new buttons are added (unless the host moved it).
    function _ctrlAppendPinned(item) {
      const last = _ctrlPinnedExtras.length - 1;
      if (last >= 0 && _ctrlPinnedExtras[last] === '_more_') {
        _ctrlPinnedExtras.splice(last, 0, item);
      } else {
        _ctrlPinnedExtras.push(item);
      }
    }
    ...
    function _ctrlRenderPinnedExtras() {
      ...
      for (let idx = 0; idx < _ctrlPinnedExtras.length; idx++) {
        ...
        if (eid === '_more_') {
          ...
          more.className = 'ctrl-bonus-btn ctrl-sect-toggle';
          more.setAttribute('data-layout-id', eid);
          more.title = 'More options';
          more.innerHTML = 'More&hellip;';
          more.onclick = _ctrlOpenExtrasPopup;
    ...
    function _ctrlRenderOrderTray() {
      ...
      // Constrain the tray to the same width the real pinned row has, so wrapping
      // here accurately shows how much the controls panel can actually hold.
      // (The controls panel has no horizontal padding, so its/the section's width
      //  is the true available width — measure it even while collapsed.)
      const ctrlPanel = document.getElementById('ctrl-controls-panel');
      const ctrlSect = document.getElementById('ctrl-section-controls');
      const availW = (ctrlPanel && ctrlPanel.clientWidth) || (ctrlSect && ctrlSect.clientWidth) || 0;
      if (availW > 0) {
        tray.style.maxWidth = availW + 'px';
        tray.style.marginLeft = 'auto';
        tray.style.marginRight = 'auto';
      } else {
        tray.style.maxWidth = '';
      }

      // Make a tray item drag-to-reorder. Mirrors the original chip drag logic.
      const _ctrlMakeTrayDraggable = (el, capturedIdx) => {
        el.draggable = true;
        el.addEventListener('dragstart', e => {
          _ctrlTrayDragSrcIdx = capturedIdx;
          e.dataTransfer.effectAllowed = 'move';
          setTimeout(() => el.classList.add('ctrl-arrange-dragging'), 0);
        });
        el.addEventListener('dragend', () => {
          el.classList.remove('ctrl-arrange-dragging');
          tray.querySelectorAll('.ctrl-arrange-drop-before,.ctrl-arrange-drop-after').forEach(x => {
            x.classList.remove('ctrl-arrange-drop-before', 'ctrl-arrange-drop-after');
          });
        });
        el.addEventListener('dragover', e => {
          ...
          el.classList.toggle('ctrl-arrange-drop-before', isLeft);
          el.classList.toggle('ctrl-arrange-drop-after', !isLeft);
        });
        el.addEventListener('dragleave', () => {
          el.classList.remove('ctrl-arrange-drop-before', 'ctrl-arrange-drop-after');
        });
        el.addEventListener('drop', e => {
          ...
          // No-op only when the drop lands exactly where the item already is:
          // onto itself, or onto the near edge of an immediate neighbour.
      ...
      // Remove the layout item at the given index (used by break/space click).
      const _ctrlRemoveTrayItem = (capturedIdx) => {
      ...
      for (let idx = 0; idx < _ctrlPinnedExtras.length; idx++) {
        ...
        // Break — full-width clickable strip, click to remove (mirrors a wrap).
        if (eid.startsWith('_br_')) {
          const el = document.createElement('div');
          el.className = 'ctrl-arrange-break';
          el.setAttribute('data-layout-id', eid);
          el.title = 'Line break — click to remove';
          el.innerHTML = '<span>&#x21B5; line break</span>';
          (function(capturedIdx) { el.onclick = () => _ctrlRemoveTrayItem(capturedIdx); })(idx);
          _ctrlMakeTrayDraggable(el, idx);
          tray.appendChild(el);
          continue;
        }
        // Space — 16px clickable strip, click to remove (same width as display).
        if (eid.startsWith('_sp_')) {
          const el = document.createElement('div');
          el.className = 'ctrl-arrange-space';
          el.setAttribute('data-layout-id', eid);
          el.title = 'Space — click to remove';
          (function(capturedIdx) { el.onclick = () => _ctrlRemoveTrayItem(capturedIdx); })(idx);
          _ctrlMakeTrayDraggable(el, idx);
          tray.appendChild(el);
          continue;
        ...
        // More… button — drag-to-reorder like any button, but never removable.
        if (eid === '_more_') {
          const more = document.createElement('button');
          more.className = 'ctrl-bonus-btn ctrl-sect-toggle ctrl-arrange-btn';
          more.title = 'More options (cannot be removed)';
          more.innerHTML = 'More&hellip;';
          more.tabIndex = -1;
          more.onclick = e => e.preventDefault();
          _ctrlMakeTrayDraggable(more, idx);
        ...
        // Reveal button has an attached variant-picker trigger; mirror the group.
        if (eid === 'tgl_peek') {
          const grp = document.createElement('div');
          grp.className = 'ctrl-queue-group ctrl-arrange-btn';
          grp.style.display = 'inline-flex';
          const btn = document.createElement('button');
          btn.className = 'ctrl-bonus-btn ' + cfg.classes + ' ctrl-queue-main';
          btn.innerHTML = cfg.html;
          ...
          trig.innerHTML = '&#x25B4;';
          trig.tabIndex = -1;
          trig.onclick = e => e.preventDefault();
          grp.appendChild(btn);
          grp.appendChild(trig);
          _ctrlMakeTrayDraggable(grp, idx);
          tray.appendChild(grp);
          continue;
        }

        // Regular button — render identically to the real display, drag-to-reorder.
        // (No data-lt-mode/data-proxy-extra so live state-sync leaves the tray static.)
        const btn = document.createElement('button');
        btn.className = 'ctrl-bonus-btn ctrl-arrange-btn ' + cfg.classes;
        btn.innerHTML = cfg.html;
        if (cfg.title) btn.title = cfg.title;
        btn.tabIndex = -1;
        btn.onclick = e => e.preventDefault();
        _ctrlMakeTrayDraggable(btn, idx);
        tray.appendChild(btn);
      }
    }

    function _ctrlClearPinned() {
      // Keep the More… button — it's the only way to reopen this menu.
      _ctrlPinnedExtras = ['_more_'];
    ...
    function _ctrlAddLayoutItem(type) {
      ...
      _ctrlAppendPinned('_' + type + '_' + idx);
      _ctrlRenderOrderTray();
      _ctrlRenderPinnedExtras();
      _ctrlSavePanels();
    }
