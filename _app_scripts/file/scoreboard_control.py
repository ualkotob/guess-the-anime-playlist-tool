...
# Default content written to "Example Rules.json" on first launch so users have
# a template to copy and edit.
_EXAMPLE_RULES = {
    "global_title": [
...
def ensure_example_rules_file():
    """Create the rules folder and an 'Example Rules.json' template if missing."""
    os.makedirs(RULES_FOLDER, exist_ok=True)
    path = os.path.join(RULES_FOLDER, "Example Rules.json")
    if os.path.exists(path):
        return
    try:
        with open(path, "w", encoding="utf-8") as f:
            json.dump(_EXAMPLE_RULES, f, indent=4, ensure_ascii=False)
    except OSError as e:
        print(f"[Rules] Could not create example rules file: {e}")
