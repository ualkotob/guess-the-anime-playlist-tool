...
def set_action_context(*, get_deduplicated_files, get_anilist_metadata):
    """Set providers used by main-menu action wrappers."""
    global _get_deduplicated_files, _get_anilist_metadata
    _get_deduplicated_files = get_deduplicated_files
    _get_anilist_metadata = get_anilist_metadata


def _current_files():
    return _get_deduplicated_files() if _get_deduplicated_files else []
...
def show_anilist_tag_stats(sort='count'):
    anilist_tag_stats(
        _current_files(),
        _get_anilist_metadata() if _get_anilist_metadata else {},
        sort,
    )
